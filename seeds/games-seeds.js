...
const TitlesPlatforms = sequelize.define('TitlesPlatforms', {
  titleId: {
    type: DataTypes.INTEGER,
    references: {
      model: Title,
...
const seedTitles = async () => {
  for (const titleData of titlesData) {
    const { platforms, ...titleWithoutPlatforms } = titleData;
    const createdTitle = await Title.create(titleWithoutPlatforms);
    await createdTitle.addPlatforms(platforms);
  }
};
